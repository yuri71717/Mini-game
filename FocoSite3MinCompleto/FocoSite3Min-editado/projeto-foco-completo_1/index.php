...
// se o cara já tiver logado e ainda estiver no tempo, manda direto pra home
...
// pega as mensagens de feedback que vêm pela URL
...
// credenciais padrão do projeto — dá pra trocar aqui
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // primeiro tenta bater com as credenciais fixas
    if (strtolower($email) === strtolower($EMAIL_PADRAO) && $senha === $SENHA_PADRAO) {
        $autenticado = true;
        $nome_usuario = 'Agente Foco';
    } else {
        // se não bater, procura no arquivo json de usuários
    ...
    // deu certo: inicia a sessão
    if ($autenticado) {
        $_SESSION['usuario'] = $nome_usuario;
        $_SESSION['email'] = $email;
        $_SESSION['login_time'] = time(); // guarda o horário do login pra controlar os 3 minutos
        $_SESSION['login_date_time'] = date('d/m/Y H:i:s'); // data legível pra mostrar na tela de resultado
